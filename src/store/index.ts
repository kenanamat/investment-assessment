...
export default createStore<RootState>({
  ...
  getters: {
    ...
    getUser: (state: RootState, getters: any) => (userid: string) => {
      const user = Object.values(getters["getUsers"]()).find(
        (u: any) => u.id == userid
      )
    ...
    getUsersInSession:
      (state: RootState, getters: any) => (sessionId: string) => {
        const users = getters["getUsers"]()
        return Object.keys(users).filter(
          (s: any) => users[s].session === sessionId
        )
      },
    ...
    validUserAndGroup:
      (state: RootState, getters: any) => (userid: string, groupid: string) => {
        return (
          getters["isValidUser"](userid) && getters["isValidGroup"](groupid)
        )
      },
      ...
      (state: RootState, getters: any) => (sessionId: string) => {
        ...
        return Object.fromEntries(
          Object.entries(groups ?? []).filter(
            ([key, val]) => (val as GroupState).session == sessionId
          )
        )
      },
    ...
    getGroupsInSession:
      (state: RootState, getters: any) => (sessionId: string) => {
        ...
        return Object.keys(groups ?? []).filter(
          (s: any) => groups[s].session == sessionId
        )
      },
    getGroupsNoUsers:
      (state: RootState, getters: any) => (sessionId: string) => {
    ...
    getGroupInputs:
      ...
      (groupId: string, currentRound: number) => {
    ...
    getGroupProfits: (state: RootState, getters: any) => (groupId: string) => {
      return getters["getGroup"](groupId).game.rounds.reduce(
        (profits: Array<number>, round: RoundState) => {
    ...
    getGroupFakeOutputs:
      (state: RootState, getters: any) =>
      (groupId: string, currentRound: number) => {
        return getters["getGroup"](groupId).fakeGame.rounds[currentRound]
          .outputs
      },
    getGroupFakeResults:
      (state: RootState, getters: any) =>
    ...
    getGroupFakeProfits:
      (state: RootState, getters: any) => (groupId: string) => {
        return getters["getGroup"](groupId).fakeGame.rounds.reduce(
          (profits: Array<number>, round: RoundState) => {
            profits.push(round.profit)
            return profits
          },
    ...
    getActiveSession: (state: RootState, getters: any) => () => {
      return (
        Object.values(getters["getSessions"]()).find(
          (s: any) => s.active == true
        ) ?? false
      )
    },
    getUsersReady: (state: RootState, getters: any) => (groupId: string) => {
      ...
      if (groupReadyUsers == undefined) {
        ...
        return Object.keys(groupReadyUsers).every(
          (u: string) => groupReadyUsers[u] === true
        )
    },
    ...
    getPathItem: (state: RootState, getters: any) => (userId: string) => {
      ...
      if (sessionPath)
        return (
          sessionPath.find(
            (p: pathItemState) => !p.completed && p.canContinue
          ) ?? false
        )
      else return false
    },
    ...
    getReadyUsers: (state: RootState, getters: any) => (groupId: string) => {
      const group = getters["getGroup"](groupId)
      return Object.keys(group.ready).filter(
        (u: string) => group.ready[u] == true
      )
    },
    getUnreadyUsers: (state: RootState, getters: any) => (groupId: string) => {
      const group = getters["getGroup"](groupId)
      return Object.keys(group.ready).filter(
        (u: string) => group.ready[u] == false
      )
    },
    ...
    getQuestionnaire:
      (state: RootState, getters: any) => (questionnaireId: string) => {
        return getters["getQuestionnaires"]()[questionnaireId] ?? false
      },
    getUserQuestionnaires:
      (state: RootState, getters: any) => (userId: string) => {
        return getters["getUser"](userId)["questionnaires"] ?? false
      },
    getUserQuestionnaire:
      (state: RootState, getters: any) =>
      (userId: string, questionnaireId: string) => {
        return (
          getters["getUserQuestionnaires"](userId)[questionnaireId] ?? false
        )
      },
    getGroupRoundQuestionnaire:
      (state: RootState, getters: any) =>
      (groupId: string, currentRound: number) => {
        return (
          getters["getGroupGameRound"](groupId, currentRound)[
            "questionnaire"
          ] ?? false
        )
      },
    ...
    getCurrentQuestion:
      (state: RootState, getters: any) =>
      (userId: string, questionnaireId: string) => {
        const questionnaire = getters["getUserQuestionnaire"](
          userId,
          questionnaireId
        )
        const currentQuestion = Object.values(questionnaire).find(
          (q: any) => q["active"] == true
        )
        return currentQuestion
      },
    getCurrentQuestionByIdx:
      (state: RootState, getters: any) =>
      (userId: string, questionnaireId: string, idx: number) => {
        const questionnaire = getters["getUserQuestionnaire"](
          userId,
          questionnaireId
        )
      ...
      return (
        capitalize(
          state.words.adjectives[
            Math.floor(Math.random() * state.words.adjectives.length)
          ]
        ) +
        capitalize(
          state.words.nouns[
            Math.floor(Math.random() * state.words.nouns.length)
          ]
        )
      )
    },
    ...
    getExcelFormat: (state: RootState, getters: any) => () => {
      ...
      Object.values(users as { [id: string]: UserState }).forEach(
        (user: UserState) => {
          ...
          if (user.questionnaires) {
            Object.values(
              user.questionnaires as { [id: string]: QuestionState[] }
            ).forEach((questionset: QuestionState[]) =>
              questionset.forEach((q: QuestionState) => {
                if (q.type === "list-scale") {
                  q.questions.forEach((listq, index) =>
              ...
              userGroup.game.rounds.forEach((round: RoundState) => {
                ...
                if (round.results) {
                  Object.assign(
                    results,
                    Object.fromEntries(
                      Object.entries(round.results).map(([k, v]) => [
                        round.index + " " + k,
                        v.toFixed(2),
                      ])
                    )
                  ...
                  Object.assign(
                    ...
                    Object.fromEntries(
                      Object.entries(round.inputs).map(([k, v]) => {
                        if (v) return [round.index + " " + k, v.toFixed(2)]
                        else return [round.index + " " + k, v]
                      })
                    )
                  )
                }
              })
              points = userGroup.game.points.toFixed(2)
            }
          }
    ...
    getTreatmentResult:
      (state: RootState, getters: any) => (groupId: string) => {
        ...
        const R_K = group.game.rounds.reduce(
          (acc: number, round: RoundState) => acc + round.inputs.R_K,
          0
        )
        const R_E = group.game.rounds.reduce(
          (acc: number, round: RoundState) => acc + round.inputs.R_E,
          0
        )
        const R_L = group.game.rounds.reduce(
          (acc: number, round: RoundState) => acc + round.inputs.R_L,
          0
        )
        ...
        const points_g =
          b_P_g * results.tot_profit_post_tax +
          b_E_g * results.tot_environmental_impact +
          b_S_g * results.tot_social_impact
        ...
        switch (group.treatment) {
          ...
          case "esg":
            return (
              points_g +
              b_RDE_t * (R_E - 320) +
              b_E_t * (results.tot_environmental_impact + 653) +
              b_S_t * (results.tot_social_impact - 50)
            )
          default:
            return 0
        }
  ...
  mutations: {
    ...
    UPDATE_GROUPREADY(state: RootState, payload) {
      if (payload.id != undefined)
        firebase
          .database()
          .ref("db/groups/" + payload.id + "/ready")
          .update(payload.user)
    },
    UPDATE_GROUPUSERS(state: RootState, payload) {
      firebase
        .database()
        .ref("db/groups/" + payload.id + "/users")
        .update(payload.user)
    },
    UPDATE_GROUPGAME(state: RootState, payload) {
      firebase
        .database()
        .ref("db/groups/" + payload.id + "/game")
        .update(payload.game)
    },
    ...
    UPDATE_SESSIONPATH(
      state: RootState,
      payload: { id: string; pathItem: string; object: {} }
    ) {
      firebase
        .database()
        .ref("db/sessions/" + payload.id + "/path/" + payload.pathItem)
        .update(payload.object)
    },
    UPDATE_GROUPROUND(
      state: RootState,
      payload: { groupId: string; currentRound: number; object: {} }
    ) {
      firebase
        .database()
        .ref(
    ...
    UPDATE_GROUPVALUE(
      ...
      firebase
        ...
        .update({ [payload.type]: payload.value })
    },
    UPDATE_GROUPANSWER(
      state: RootState,
      payload: { groupId: string; currentRound: number; answers: {} }
    ) {
      firebase
        .database()
        .ref(
  ...
  actions: {
    ...
    addUser(context, payload: UserState) {
      context.commit("UPDATE_USER", payload)
    },
    removeLocalUser(context, localUserid) {
    ...
    initiateUser(context: ActionContext<RootState, RootState>, userid: string) {
      rootDatabase.app
        .auth()
        .signInAnonymously()
        .then(() => {
    ...
    createUser(context, sessionId: string) {
      // create username
    ...
    addUserToGroup(context, userId: string) {
      // get max per group
      const sessionId = context.getters["getActiveSession"]().id
      ...
      if (gid && context.getters["getGroup"](gid).users != undefined) {
        while (
          context.getters["getGroup"](gid).users != undefined &&
          Object.keys(context.getters["getGroup"](gid).users).length >=
            maxUsersPerGroup
        ) {
      ...
      // create leader
      if (context.getters["getGroup"](gid).leader == "")
        context.commit("UPDATE_GROUP", { id: gid, leader: userId })
    ...
    startSession(
      ...
      const sameDaySessions = context.getters["getSessionsList"]().filter(
        (s: string) => s.includes(dateToday)
      )
    ...
    unreadyUser(context, user: string) {
      context.commit("UPDATE_GROUPREADY", {
        id: context.getters["getUserGroup"](user).id,
        user: { [user]: false },
      })
    },
    checkPath(context) {
    ...
    addQuestionnaireToUser(
      ...
      if (context.getters["getUserQuestionnaire"](uid, qid) || uid == "admin") {
        ...
        if (user.questionnaires == undefined)
          adjustedQuestionnaire = { [qid]: questionnaire }
        else
          adjustedQuestionnaire = Object.assign(user.questionnaires, {
            [qid]: questionnaire,
          })
    ...
    gotoNextQuestion(
      ...
      if (
        Number(payload.currentQuestionId) ==
        Object.keys(
          context.getters["getUserQuestionnaire"](
            payload.userId,
            payload.questionnaire
          )
        ).length
    ...
    readyUp(context, groupId: string) {
      const usersInGroup = Object.keys(
        context.getters["getGroup"](groupId).users
      )
    ...
    setTimerEnd(context, seconds: number) {
      const session = context.getters["getActiveSession"]()
      context.commit("UPDATE_SESSION", {
        id: session.id,
