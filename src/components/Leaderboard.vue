<template>
  ...
  <div id="score-table">
    ...
    <div class="row" v-for="group in groupsInSessionIds">
      ...
      <div class="col border" v-for="profit in profitsGroups[group].slice(0, currentRound + 1)">
        {{profit}}
      </div>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  const profitsGroupsDataset = computed(() => groupsInSessionIds.reduce((profits: Array<Object>, groupId: string) => {
    profits.push( {
      label: groupId,
  ...
  const profitsGroups = computed(() => groupsInSessionIds.reduce((profits: {[id: string]: number}, groupId: string) => {
