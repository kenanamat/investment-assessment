<template>
  <div id="leaderboard">
        ...
        <div class="col-3">
          <div class="scores-list">
            ...
            <div v-for="group in rankedProfits.slice(0, 5)" :key="group[0]">
              ...
              {{
                Math.round(
                  (group[1] as GroupState).fakeGame.rounds[currentRound].results
                    .tot_profit_post_tax
                )
              }}
            </div>
    ...
    <h4 v-if="Object.values(userGroup.ready)[0]">
      Please wait until the other groups are also ready, then the next round
      will start
    </h4>
  </div>
</template>
...
const groups = computed(() =>
  store.getters["getGroupsFromSession"](activeSession.id)
)
...
const getDataProfit = computed(() => {
  return Array.from(
    Object.values(
      groups.value[selectedGroup.value].fakeGame.rounds
    ) as RoundState[],
...
const rankedRD = computed(() => {
  return Object.entries(groups.value).sort(
    ([, a], [, b]) =>
      (b as GroupState).fakeGame.rounds[currentRound.value].results.tot_rd -
      (a as GroupState).fakeGame.rounds[currentRound.value].results.tot_rd
  )
})
